...
- 将该功能移入 LZR 库
- 思考避免客户端多开的方法
- 在改版后的 OpenShift3 上重新测试单通道对接状况
- 在改版后的 OpenShift3 上重新测试客户端在不设 Content-Length 的情况下，可否持续发送数据
- 使用 ws 进行 WebSocket 连接测试
- 研究 node.js 并发连接数大于 7 个时其它连接被排队阻塞的原理
...
##### 2019-1-17 （ 内网渗透完成 ）：
- O3改版以后，网速很稳定，在这种环境下，单发模式运行基本正常，暂未发现问题。

##### 2019-1-17 （ 改回单发模式 ）：
	改回单通道单向发送请求，HTTP接收应答的方式。

##### 2019-1-16 （ 放弃单通道 ）：
- 单通道测试总结：
	- 在网速不佳的情况下，对接效果并不理想，时常掉线，重连困难。
	- 在网速较好的情况下，勉强可用。

##### 2019-1-16 （ 精简对接过程 ）：
- 重连效果不理想的原因，是由于客户端多开所致！

##### 2019-1-15 （ 重连效果不理想 ）：
- 只有初次连接较为稳定，重连之后问题特别多，多得无法控制：
	- 客户端已被强迫中断，但服务端依然可继续发送数据。
	- 客户端重连成功后，服务端滞压的数据会被他同时传回。

##### 2019-1-15 （ 解决重连对接中的问题 ）：
- 可重连单通道测试结果：
	- 连接中存在超时问题。超时后，服务端不会立即断开连接，导致客户端的重连失败。
		- 解决办法 ： 无论是否对接，都需要对对接指令进行处理
	- 已对接好的连接，在服务端会被篡改为新的连接来处理。
		- 解决办法 ： 检查新连接的信息是否属于三类对接交互信息之一。
		- 新解决办法 ： 此问题的根本原因可能是客户端多开所致，应避免客户端多开。
